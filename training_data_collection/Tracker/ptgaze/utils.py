...
def _generate_dummy_camera_params(config: yacs.config.CfgNode) -> None:
    ...
    if config.cur.image_path:
        path = pathlib.Path(config.cur.image_path).expanduser()
        image = cv2.imread(path.as_posix())
        h, w = image.shape[:2]
    elif config.cur.video_path:
        logger.debug(f'Open video {config.cur.video_path}')
        path = pathlib.Path(config.cur.video_path).expanduser().as_posix()
        cap = cv2.VideoCapture(path)
        cap.set(5, 60)
        if not cap.isOpened():
            raise RuntimeError(f'{config.cur.video_path} is not opened.')
    ...
    logger.debug(f'Close video {config.cur.video_path}')
...
def _update_camera_config(config: yacs.config.CfgNode) -> None:
    ...
    if not config.gaze_estimator.camera_params:
        ...
        logger.debug(f'config.cur.use_camera is {config.cur.use_camera}')
        if config.cur.use_camera:
        ...
        elif config.cur.image_path or config.cur.video_path:
            ...
            logger.debug('Bothe config.cur.image_path and '
                         'config.cur.video_path are not specified.')
            raise ValueError(
                'No input found. config.cur.use_camera is False and '
                'both config.cur.image_path and config.cur.video_path '
                'are not specified.')
...
def _expanduser_all(config: yacs.config.CfgNode) -> None:
    ...
    config.cur.image_path = _expanduser(config.cur.image_path)
    config.cur.video_path = _expanduser(config.cur.video_path)
    config.cur.output_dir = _expanduser(config.cur.output_dir)
...
def _check_path_all(config: yacs.config.CfgNode) -> None:
    ...
    if config.cur.image_path:
        _check_path(config, 'cur.image_path')
    if config.cur.video_path:
        _check_path(config, 'cur.video_path')
...
def update_default_config(config: yacs.config.CfgNode,
    ...
    if args.image:
        ...
        config.cur.image_path = args.image
        config.cur.use_camera = False
        logger.debug(
            f'Update config.cur.image_path to {config.cur.image_path}')
        logger.debug('Set config.cur.use_camera False')
    if args.video:
        logger.debug(f'--video is {args.video}')
        config.cur.video_path = args.video
        config.cur.use_camera = False
        logger.debug(
            f'Update config.cur.video_path to {config.cur.video_path}')
        logger.debug('Set config.cur.use_camera False')
    if args.output_dir:
        logger.debug(f'--output-dir is {args.output_dir}')
        config.cur.output_dir = args.output_dir
        logger.debug(f'Update config.cur.output_dir')
    if args.ext:
        logger.debug(f'--ext is {args.ext}')
        config.cur.output_file_extension = args.ext
        logger.debug('Update config.cur.ouput_file_extension')
    if args.no_screen:
        logger.debug(f'--no-screen is set')
        config.cur.display_on_screen = False
        logger.debug('Set config.cur.display_on_screen False')
        if not config.cur.output_dir:
            logger.debug('config.cur.output_dir is not specified.')
            config.cur.output_dir = 'outputs'
            logger.debug('Set config.cur.output_dir \'outputs\'')
